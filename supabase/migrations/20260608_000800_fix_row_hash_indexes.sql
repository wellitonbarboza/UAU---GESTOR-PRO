...
-- Guarantee deduplication indexes for all sheet tables used by import_uau_sheets
...
alter table if exists public."549-ITENS DOS CONTRATOS" add column if not exists row_hash text;
...
alter table if exists public."260-DESEMBOLSO DET. PRODUTO" add column if not exists row_hash text;
...
alter table if exists public."CUSTO AO TERMINO" add column if not exists row_hash text;
...
alter table if exists public."INCC" add column if not exists row_hash text;
create unique index if not exists "incc_row_hash_idx" on public."INCC"(obra_id, row_hash) where row_hash is not null;

alter table if exists public."DE-PARA ORCAMENTO" add column if not exists row_hash text;
